...
In the Next.js frontend, you can collapse the sidebar using the control inside the sidebar itself to free more space for the chat area. The workspace selector is available at the bottom of the sidebar. In expanded mode, the sidebar shows a fuller left navigation panel. In collapsed mode, it changes into a slim rail with quick file and session buttons.

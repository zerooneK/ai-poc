...
The Next.js shell now also supports a collapsible sidebar, with a Gemini-like expanded left panel, a slim icon rail when collapsed, the collapse control inside the sidebar itself, and the workspace selector anchored at the bottom. A browser reload in that frontend now creates a fresh session instead of reviving the previously active one from local storage.

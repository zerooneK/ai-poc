...
Note: the in-progress Next.js frontend also includes a client-side collapsible sidebar. The collapse control lives inside the sidebar, the workspace selector is anchored at the bottom, the expanded state uses a fuller left navigation panel, and the collapsed state uses a slim icon rail. That frontend now also starts a fresh client session on browser reload instead of restoring the previous one automatically. These are presentation/session-bootstrap changes only and do not alter backend routes or request formats.
